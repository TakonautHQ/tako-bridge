...
function deps(responses: HttpResult[]): {
	deps: DeviceDeps;
	calls: string[];
	logs: string[];
} {
...
describe("runDeviceLogin", () => {
	...
	it("throws on a non-pending error from token", async () => {
		const { deps: d } = deps([
			{ status: 400, json: { detail: "expired_token" } },
		]);
		await expect(runDeviceLogin("https://x", d)).rejects.toThrow(
			/device\/token failed/,
		);
	});
	...
	it("times out if never approved", async () => {
		...
		await expect(runDeviceLogin("https://x", d, 3)).rejects.toThrow(
			/Timed out/,
		);
	});
