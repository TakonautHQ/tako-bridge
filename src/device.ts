...
export interface DeviceDeps {
  fetchJson: (method: string, path: string, body?: unknown) => Promise<HttpResult>;
...
export async function runDeviceLogin(
  ...
  const { device_code, user_code, interval, verification_uri, verification_uri_complete } = start.json;
  ...
  deps.log("");
  if (url) {
    deps.log("To connect, open this link, review the displayed organization, and confirm:");
    deps.log(`  ${url}`);
    if (bareVerificationUrl) {
      deps.log(`Or, on any device, go to ${bareVerificationUrl} and enter code:  ${user_code}`);
    }
    deps.openUrl?.(url);
  } else {
  ...
  for (let i = 0; i < maxPolls; i++) {
    const tok = await deps.fetchJson("POST", "/api/auth/device/token", { device_code, label });
    if (tok.status === 200) {
    ...
    throw new Error(`device/token failed: HTTP ${tok.status} ${JSON.stringify(tok.json)}`);
  }
